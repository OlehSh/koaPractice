...
import { NODE_LABEL, RELATION_LABEL, QueryParams } from "./interfase";
  ...
  async add(token: string, userId: string): Promise<SessionInfo> {
    ...
    try {
      ...
      const queryResult = await tx.run(
        `MATCH (u:${NODE_LABEL.PROFILE} { id: $userId}) CREATE (t:${NODE_LABEL.TOKEN} {id: $id, token: $token, myVar: 'test', active: $active})<-[r:${RELATION_LABEL.SESSION}]-(u) RETURN t`,
